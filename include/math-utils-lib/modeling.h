...
// ===========================================================================================================
// WARNING: EIGEN-ONLY TEMPLATES!

template <typename xT>
class RK4
{
private:
    boost::function<void(Ref<VectorXd>, const Ref<const VectorXd>&, const Ref<const VectorXd>&)> f_;
    xT k1_;
    xT k2_;
    xT k3_;
    xT k4_;
...
public:
    ...
    RK4(void (*f)(Ref<VectorXd>, const Ref<const VectorXd>&, const Ref<const VectorXd>&))
    {
        f_ = boost::bind(f, _1, _2, _3);
        initialized_ = true;
    }
    template <class C>
    RK4(void (C::*f)(Ref<VectorXd>, const Ref<const VectorXd>&, const Ref<const VectorXd>&), C *obj)
    {
        f_ = boost::bind(f, obj, _1, _2, _3);
        initialized_ = true;
    }
    void setDynamics(void (*f)(Ref<VectorXd>, const Ref<const VectorXd>&, const Ref<const VectorXd>&))
    {
        f_ = boost::bind(f, _1, _2, _3);
        initialized_ = true;
    }
    template <class C>
    void setDynamics(void (C::*f)(Ref<VectorXd>, const Ref<const VectorXd>&, const Ref<const VectorXd>&), C *obj)
    {
        f_ = boost::bind(f, obj, _1, _2, _3);
        initialized_ = true;
    }
    void run(Ref<VectorXd> x, const Ref<const VectorXd> &u, const double &dt)
    {
        if (initialized_)
        {
            f_(k1_, x, u);
